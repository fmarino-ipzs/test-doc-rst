...
The ``OAuth-Client-Attestation`` is signed using the private key bound to the Wallet Instance. The related public key that is attested by the Wallet Provider is provided within the Wallet Attestation (``cnf.jwk`` claim). The Credential Issuer performs the following checks on the Token Request:
   ...
   1. It MUST ensure that the Authorization ``code`` is issued to the authenticated Wallet Instance and was not replied.
   2. It MUST ensure the Authorization ``code`` is valid and has not been previously used .
   3. It MUST ensure the ``redirect_uri`` matches the value included in the previous Request Object.
   4. It MUST validate the DPoP Proof JWT, according to Section 4.3.
